...
class SPageFilePhysics(ctypes.Structure):
    ...
    _fields_ = [
        ('packetId', c_int32),
        ('gas', c_float),           # throttle -> input
        ('brake', c_float),         #          -> input
        ('gear', c_int32),          # gear     -> output
        ('fuel', c_float),          # fuel     -> output √
        ('rpms', c_int32),          # rpm      -> output √
        ('steerAngle', c_float),    #          -> input
        ('speedKmh', c_float),      #          -> output √
        ('velocity', c_float * 3),  #          -> output 
        ('accG', c_float * 3),      #          √
        ('wheelSlip', c_float * 4), #          
        ('wheelLoad', c_float * 4),
        ('wheelsPressure', c_float * 4),
        ('wheelAngularSpeed', c_float * 4),    #    -> output
        ('tyreWear', c_float * 4),
        ...
        ('drsEnabled', c_int32),
        ('brakeTemp', c_float*4),
        ('clutch', c_float),            # -> input
        ('tyreTempI', c_float*4), #       -> output
        ('tyreTempM', c_float*4), # -> output
        ('tyreTempO', c_float*4), # -> output
